...
Finally you have to override the \verb+parameter.xml+ file already present in\\  
...
The current version of the model has no runtime monitoring graphical element, so one can check the progress of the simulation watching at the tick count number in upper right corner of the RS GUI window or at the text output in the console panel of Eclipse window as shown in the following figure. 
...
\verb+batch_date+. Needless to say that the sub-folder name can be chosen at your convenience, and it can be changed at each batch run in order to avoid overriding the data.
...
Throughout this document we will extensively use the two visual representations reported in figures \ref{fig:components} and \ref{fig:sequence}. The first one aims at showing the \textbf{model components} and the second one describes the dynamics of events. This includes the initialization phase who is performed just once at the beginning of the simulation and the \textbf{sequence of events} repeated in each time step.
...
\chapter{The components of the system}
...
\newpage
\chapter{The Dynamics of Events}
