...
\part{Modifying and developing GABRIELE}


\chapter{Modifying the code}

Modifying an existing code is a demanding task because the developer needs information distributed in several thousands lines of code.

Even an open source code ends up to be proprietary if it is not properly documented.

Because GABRIELE is provided with a collaborative intent, a particular care was dedicated to providing a documentation allowing new developers to easily understand where the code has to be modified.

\section{Learning the current state of development}
The reading of the above material of this document gives the idea of the state of development. However, developers need something more detailed. This more detailed information is included in the \verb+docs+ folder of GABRIELE distribution.  

Opening the \verb+index.html+ file using a browser, a web page with four sections is displayed.

The four sections are
\begin{itemize}
	\item PDF;
	\item Relationships among agents (classes)
	\item UML activity diagrams for the sequence of events
	\item javadoc API
\end{itemize}

The PDF section points to the present manual. Javadoc API documentation is standard, while the other two items represents unusual efforts in software documentation.

The relationship among classes presents an UML diagram showing GABRIELE classes and the relationships among them. This represents a useful hint for understanding the software components, however, the bulk of the insights can be obtained in the UML activity diagrams for the sequence of events section.

The latter presents the sequence of actions for both the initialization (giving a more detailed description of what presented in Section \ref{sec:initialization}) and the main loop (giving a more detailed description of what presented in Section \ref{sec:mainloop}).

For each of these action a link to an UML activity diagram is provided. UML activity diagrams give a visual representation of the code and allow developers to easily pinpoint where the code should be modified in order to obtain the desired result.


\chapter{Producing documentation: \LaTeX, javadoc and UML}
...
\subsubsection{Documenting a Class}
...
The following figure gives an idea of the outcome. It is obtained by running \verb+UmlGraph+ on the \verb+Government.java+ file

\includegraphics[scale=0.5]{../umlgraph/government_class_diagram.jpg}


\subsubsection{Documenting packages}

To document classes in packages and the relationships among them, the \verb+UmlGraph+ docled has to be used as above but with different inputs.
...
The following figure gives an idea of this process outcome. It is obtained by running \verb+UmlGraph+ on the \verb+gabriele, gabriele.agents, gabriele.institutions+ packages
...
The \verb+UmlGraphDoc+ docled has to be used instead of the \verb+UmlGraph+ doclet to integrate javadoc output with UML graph showing the relationships of a class with the rest of the code .
...
-sourcepath <path to the packages source files>
-docletphath <path to umlgraph jar file>
-doclet org.umlgraph.doclet.UmlGraphDoc <options> <packages separated by a white space>
\end{verbatim}

Among the available options, \verb+-collapsible+ is perhaps one worth to be supplied.

It inserts in the java documentation the button \raisebox{-1mm}{\includegraphics[scale=0.4]{fig_gabriele_umlgraph.png}}.
...
Activity diagrams are used to describe how the code implements simulations events.

An activity diagram is build for each one of the item reported in figure \ref{fig:sequence}. These diagrams are gathered in the \verb+gabriele/docs/umldoc+ folder.

When a developer add an event, s/he is asked to provide the diagram for the event and integrate it in the current documentation.

The existing diagrams were made using The Umlet software. 
...
The Umlet graphical user interface is displayed in the following figure
...
The text to be supplied is a pseudo code written using Umlet syntax. 
Using this tool, the user is able to produce the graph which shows how the action is implemented in the code. As an example, we report hereafter the diagram for the \verb+activateOfficeForLabor+ action. 
